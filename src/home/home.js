...
function contextMenuClick (args) {
    ...
    switch (commandType.toLowerCase()) {
        ...
        case 'addcustomproperty':
            ...
            let custom = '';
            if (node.addInfo) {
                for(let p in node.addInfo) {
                    custom += `<li id="${p}${node.addInfo[p]}" style="margin-bottom:10px;">${p}: ${node.addInfo[p]} <span style="float:right;"><button onclick="removeCustomProp('${p}', '${node.addInfo[p]}')">X</button></span></li>`
                }
            }
            else
                node.addInfo = {};
            var container = document.getElementById('custom-properties-dialog');
            if (!container.children.length) {
                var dialog = new ej.popups.Dialog({
                    visible: true,
                    content:
                    `<p>Add Custom Property</p><ul id="owl-custom-props" style="margin:10px 0 25px;">${custom}</ul><input id="owl-custom-prop-key" class="e-input" type="text" placeholder="Property name"/><input id="owl-custom-prop-value" class="e-input" type="text" placeholder="Value"/>`,
                    buttons: [
                        {
                            // Click the footer buttons to hide the Dialog
                            click: function () {
                                var node = diagram.selectedItems.nodes[0];
                                var prop = document.getElementById('owl-custom-prop-key');
                                var val = document.getElementById('owl-custom-prop-value');
                                if (prop.value && val.value) {
                                    node.addInfo[prop.value] = val.value;
                                }
                                prop.value = '';
                                val.value = ''; 
                                dialog.hide();
                ...
                dialog.appendTo('#custom-properties-dialog');
            }
            else {
                document.getElementById('owl-custom-props').innerHTML = custom;
                container.ej2_instances[0].show();
            }
            break;
        }
}

function removeCustomProp(prop, val) {
    var node = diagram.selectedItems.nodes[0];
    delete node.addInfo[prop];
    document.getElementById(prop + val).style.display = 'none';
}   
